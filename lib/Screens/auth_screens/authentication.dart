...
class _AuthenticateState extends State<Authenticate> {
      ...
      child = Material(
          ...
          child: Center(
            ...
            child: Text(
              ...
              style: TextStyle(color: starchatWhite, fontSize: 18, height: 1.5),
            ),
          )));
    else {
      child = Container();
    }
    return starchat.getNTPWrappedWidget(child);
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    if (enteredPasscode.length == 4) {
      bool isValid =
          starchat.getHashedAnswer(enteredPasscode) == widget.passcode;
      ...
      if (isValid) {
        ...
        if (passcodeTries > 3) {
          starchat.toast('Try after ${math.pow(2, passcodeTries - 3)} minutes');
          starchat.toast(getTranslated(this.context, 'authfailed'));
          widget.state.pop();
        }
      }
    }
