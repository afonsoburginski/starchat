...
import 'package:starchat/Screens/auth_screens/login.dart';
import 'package:starchat/Screens/chat_screen/utils/uploadMediaWithProgress.dart';
import 'package:starchat/Screens/contact_screens/SelectContactsToForward.dart';
import 'package:starchat/Screens/security_screens/security.dart';
import 'package:starchat/Services/Admob/admob.dart';
import 'package:starchat/Utils/mime_type.dart';
import 'package:starchat/main.dart';
import 'package:starchat/widgets/CountryPicker/CountryCode.dart';
import 'package:starchat/Configs/Dbkeys.dart';
import 'package:starchat/Configs/Dbpaths.dart';
import 'package:starchat/Screens/chat_screen/utils/deleteChatMedia.dart';
...
import 'package:starchat/widgets/MultiDocumentPicker/multiDocumentPicker.dart';
import 'package:starchat/widgets/MultiImagePicker/multiImagePicker.dart';
import 'package:starchat/widgets/MyElevatedButton/MyElevatedButton.dart';
import 'package:starchat/widgets/SoundPlayer/SoundPlayerPro.dart';
...
import 'package:starchat/Screens/chat_screen/utils/audioPlayback.dart';
import 'package:starchat/Screens/chat_screen/utils/downloadMedia.dart';
import 'package:starchat/Screens/chat_screen/utils/message.dart';
import 'package:starchat/Screens/contact_screens/ContactsSelect.dart';
import 'package:starchat/Models/DataModel.dart';
import 'package:starchat/Screens/chat_screen/utils/photo_view.dart';
import 'package:starchat/Screens/profile_settings/profile_view.dart';
import 'package:starchat/Services/Providers/seen_provider.dart';
import 'package:starchat/Services/Providers/seen_state.dart';
import 'package:starchat/Screens/calling_screen/pickup_layout.dart';
import 'package:starchat/Utils/call_utilities.dart';
import 'package:starchat/Utils/permissions.dart';
import 'package:starchat/Utils/chat_controller.dart';
import 'package:starchat/Utils/crc.dart';
...
import 'package:starchat/widgets/AudioRecorder/Audiorecord.dart';
import 'package:starchat/widgets/ImagePicker/image_picker.dart';
import 'package:starchat/widgets/VideoPicker/VideoPicker.dart';
import 'package:starchat/widgets/VideoPicker/VideoPreview.dart';
import 'package:starchat/Screens/chat_screen/Widget/bubble.dart';
...
import 'package:starchat/Models/E2EE/e2ee.dart' as e2ee;
import 'package:starchat/Utils/unawaited.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  String decryptWithCRC(String input) {
    try {
      ...
      starchat.toast(getTranslated(this.context, 'msgnotload'));
      return '';
    }
    starchat.toast(getTranslated(this.context, 'msgnotload'));
    return '';
  ...
  readLocal(
    ...
    chatId = starchat.getChatId(currentUserNo, peerNo);
    ...
    if (content.trim() != '') {
      try {
        ...
        if (encrypted is String) {
          ...
          starchat.toast('Nothing to encrypt');
        }
  ...
  contextMenuNew(contextForDialog, Map<String, dynamic> mssgDoc, bool isTemp,
    ...
    if ((mssgDoc[Dbkeys.from] == currentUserNo &&
      ...
      tiles.add(ListTile(
          ...
          onTap: () async {
            starchat.toast(getTranslated(contextForDialog, 'deleting'));
            await FirebaseFirestore.instance
                .collection(DbPaths.collectionmessages)
                .doc(chatId)
                .collection(chatId!)
                ...
                .then((chatDoc) async {
              if (!chatDoc.exists) {
                starchat.toast('Please reload this screen !');
                  ...
                  if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true
                    ...
                    await deleteMsgMedia(realtimeDoc, chatId!)
                      ...
                      if (isDeleted == false || isDeleted == null) {
                        starchat.toast('Could not delete. Please try again!');
                      } else {
                        await FirebaseFirestore.instance
                            .collection(DbPaths.collectionmessages)
                            .doc(chatId)
      ...
      if (list[index].data()!.containsKey(Dbkeys.groupNAME)) {
        ...
        try {
          ...
          if (encrypted is String) {
            ...
            if (content.trim() != '') {
              ...
              if (targetPeer == null) {
                await ChatController.request(
                    ...
                    starchat.getChatId(widget.currentUserNo,
                        list[tempSendIndex][Dbkeys.phone]));
              }
            ...
            starchat.toast('Nothing to send');
          }
        } catch (e) {
          setStateIfMounted(() {
            isgeneratingSomethingLoader = false;
          });
          starchat.toast('Failed to Forward message. Error:$e');
        }
      }
    }
  }
                          ...
                          Container(
                              child: Row(
                                ...
                                Icon(
                                  ...
                                  size: 12,
                                  color: starchatGrey.withOpacity(0.5),
                                ),
                                SizedBox(
                                  width: 5,
                                ),
                                Text(getTranslated(this.context, 'forwarded'),
                                    maxLines: 1,
                                    style: TextStyle(
                                        color: starchatGrey.withOpacity(0.7),
          ...
          FlatButton(
              onPressed: () async {
                ...
                if (isUser == null || isUser == false) {
                  starchat.toast(getTranslated(this.context, 'usernotjoined') +
                      ' $Appname');
                }
              },
  ...
  replyAttachedWidget(BuildContext context, var doc) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(
              color: starchatWhite.withOpacity(0.55),
          ...
          child: Stack(
            children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      color: starchatGrey.withOpacity(0.1),
      ...
      child: isLoading
          ? Container(
              ...
              color: DESIGN_TYPE == Themetype.whatsapp
                  ? starchatBlack.withOpacity(0.0)
                  : starchatWhite.withOpacity(0.0),
            )
          : Container(),
    );
  }
    ...
    return Positioned(
      child: isgeneratingSomethingLoader
          ? Container(
              ...
              color: DESIGN_TYPE == Themetype.whatsapp
                  ? starchatBlack.withOpacity(0.6)
                  : starchatWhite.withOpacity(0.6),
            )
          : Container(),
    );
  }
                            ...
                            Navigator.push(
                              ...
                              if (url != null) {
                                ...
                                onSendMessage(
                                    ...
                                    thumnailtimestamp);
                                starchat
                                    .toast(getTranslated(this.context, 'sent'));
                              } else {}
                            });
                          },
                          elevation: .5,
    ...
    if (chatStatus == ChatStatus.requested.index) {
      return AlertDialog(
        ...
        title: Text(
          ...
          style: TextStyle(color: starchatBlack),
        ),
        actions: <Widget>[
          // ignore: deprecated_member_use
          FlatButton(
          ...
          FlatButton(
              child: Text(getTranslated(this.context, 'acpt'),
                  style: TextStyle(color: starchatgreen)),
              onPressed: () {
                      ...
                      children: [
                        ...
                        Flexible(
                          child: TextField(
                            ...
                            style:
                                TextStyle(fontSize: 16.0, color: starchatBlack),
                    ...
                    child: IconButton(
                      icon: new Icon(
                        textEditingController.text.length == 0 ||
                            ...
                            ? Icons.mic
                            : Icons.send,
                        color: starchatWhite.withOpacity(0.99),
                      ),
                      onPressed: observer.ismediamessagingallowed == true
                          ? textEditingController.text.length == 0 ||
                              ...
                              : observer.istextmessagingallowed == false
                                  ? () {
                                      starchat.showRationale(getTranslated(
                  ...
                  child: SizedBox(
                    ...
                    child: EmojiPicker(
                        ...
                        config: Config(
                            ...
                            indicatorColor: starchatgreen,
                            iconColor: Colors.grey,
                            iconColorSelected: starchatgreen,
                            progressIndicatorColor: Colors.blue,
                            backspaceColor: starchatgreen,
                            showRecentsTab: true,
                            recentsLimit: 28,
  ...
  //                     textCapitalization: TextCapitalization.sentences,
  //                     maxLines: null,
  //                     style: TextStyle(fontSize: 18.0, color: starchatBlack),
  //                     controller: textEditingController,
  //                     decoration: InputDecoration(
  //                       enabledBorder: OutlineInputBorder(
  //                         // width: 0.0 produces a thin "hairline" border
...
//-- GROUP BY DATE ---
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      scaffold: starchat.getNTPWrappedWidget(WillPopScope(
                                    ...
                                    children: [
                                      Padding(
                                        padding: const EdgeInsets.fromLTRB(
                                            0, 7, 0, 7),
                                        child:
                                            starchat.avatar(peer, radius: 20),
                                      ),
                                      SizedBox(
                                        width: 7,
                                      ),
                                        ...
                                        onSelected: (dynamic val) {
                                          switch (val) {
                                            ...
                                            case 'remove_wallpaper':
                                              ...
                                              // starchat.toast('Wallpaper removed.');
                                              break;
                                            case 'set_wallpaper':
                                              Navigator.push(
                                                  context,
