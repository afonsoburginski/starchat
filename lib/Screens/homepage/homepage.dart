...
import 'package:starchat/Screens/Broadcast/AddContactsToBroadcast.dart';
import 'package:starchat/Screens/Groups/AddContactsToGroup.dart';
import 'package:starchat/Screens/SettingsOption/settingsOption.dart';
import 'package:starchat/Screens/homepage/Setupdata.dart';
...
import 'package:starchat/Screens/sharing_intent/SelectContactToShare.dart';
import 'package:starchat/Screens/splash_screen/splash_screen.dart';
import 'package:starchat/Screens/status/status.dart';
...
import 'package:starchat/Services/localization/language.dart';
import 'package:starchat/Utils/phonenumberVariantsGenerator.dart';
...
import 'package:starchat/Screens/profile_settings/profileSettings.dart';
import 'package:starchat/main.dart';
import 'package:starchat/Screens/recent_chats/RecentsChats.dart';
import 'package:starchat/Screens/search_chats/SearchRecentChat.dart';
...
class HomepageState extends State<Homepage>
  ...
  //       starchatWrapper.setLocale(context, _locale);
  //       setState(() {});
  //     }
  //   }).catchError((onError) {
  //     starchat.toast(
  ...
  triggerSharing() {
    ...
    if (_sharedText != null) {
      ...
      if (_sharedFiles!.length > observer.maxNoOfFilesInMultiSharing) {
        starchat.toast(getTranslated(context, 'maxnooffiles') +
            ' ' +
  ...
  logout(BuildContext context) async {
    ...
    Navigator.of(context).pushAndRemoveUntil(
      MaterialPageRoute(
        builder: (BuildContext context) => starchatWrapper(),
      ),
      (Route route) => false,
    );
  }
  ...
  void listenToNotification() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      if (message.data['title'] != 'Call Ended' &&
        ...
        starchat.toast(getTranslated(this.context, 'newnotifications'));
      } else {
...
//  These firestore documents will be automatically in the first run set if Admin app is required but not configured yet. You need to edit all the default settings through admin app-----
      ...
      if (value == true) {
        await writeRequiredNewFieldsAllExistingUsers().then((result) async {
          if (result == true) {
            ...
            starchat
                .showRationale(getTranslated(this.context, 'loadingfailed'));
          } else {
        ...
        starchat
            .showRationale(getTranslated(this.context, 'failedtoconfigure'));
      }
    });
  }
      ...
      await FirebaseFirestore.instance
        ...
        if (doc.exists && doc.data()!.containsKey(Dbkeys.usersidesetupdone)) {
          if (!doc.data()!.containsKey(Dbkeys.updateV7done)) {
            ...
            starchat.toast(getTranslated(this.context, 'erroroccured'));
          } else {
            setState(() {
                    ...
                    builder: (BuildContext context) {
                      ...
                      return new WillPopScope(
                          ...
                          child: AlertDialog(
                            title: Text(
                              title,
                              style: TextStyle(color: starchatDeepGreen),
                            ),
                            ...
                            actions: <Widget>[
                              TextButton(
                                  child: Text(
                                    btnLabel,
                                    style: TextStyle(color: starchatLightGreen),
                                  ),
  ...
  Future<bool> onWillPop() {
    ...
    if (now.difference(currentBackPressTime!) > Duration(seconds: 3)) {
      ...
      starchat.toast('Double Tap To Go Back');
                                                                                  ...
                                                                                  Navigator.pushReplacement(
                                                                                      ...
                                                                                      new MaterialPageRoute(
                                                                                          builder: (context) => ProfileSetting(
                                                                                                ...
                                                                                                type: starchat.getAuthenticationType(biometricEnabled, _cachedModel),
                                                                                              )));
                                                                                },
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                  fontSize: 17,
                  color: starchatWhite.withOpacity(0.7),
