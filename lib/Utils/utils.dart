...
class starchat {
  ...
  static void toast(String message) {
    Fluttertoast.showToast(
        msg: message,
        backgroundColor: starchatBlack.withOpacity(0.95),
  ...
  static void internetLookUp() async {
    try {
      await InternetAddress.lookup('google.com').catchError((e) {
        starchat.toast('No internet connection ${e.toString()}');
      });
    } catch (err) {
      starchat.toast('No internet connection. ${err.toString()}');
    }
  }
      ...
      if (user![Dbkeys.aliasAvatar] == null)
        return (user[Dbkeys.photoUrl] ?? '').isNotEmpty
            ? CircleAvatar(
                ...
                radius: radius)
            : CircleAvatar(
                backgroundColor: starchatgreen,
                foregroundColor: Colors.white,
                child: Text(getInitials(starchat.getNickname(user)!)),
                radius: radius,
              );
      return CircleAvatar(
                  ...
                  child: Center(
                      child: Padding(
                          ...
                          child: Text(
                            ...
                            style:
                                TextStyle(color: starchatWhite, fontSize: 18),
                          ))));
          }
          return child;
        });
